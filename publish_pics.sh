...
for img in $(${PROG_EXIFTOOL} -m -q -q -if '$rating' -p '$filename' ${SOURCE_DIR}/(#i)(*{png,mov,mp4,jpg})*(N)); do
	...
	if [[ ${DRY} -eq 0 ]]; then
		if [[ -n $ISIMAGE ]]; then
			...
			${PROG_SETFILE} -d "$(${PROG_GETFILEINFO} ${SOURCE_DIR}/${img} | grep created | ${PROG_CUT} -c10-)" "${EXPORT_DIR}/${img}"
			...
			${PROG_SETFILE} -d "$(${PROG_GETFILEINFO} ${SOURCE_DIR}/${img} | grep created | ${PROG_CUT} -c10-)" "${EXPORT_DIR}/${img}-x264_aac.mp4"
